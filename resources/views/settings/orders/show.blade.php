				...
				<div class="row">
					<div class="col-12 col-md-6">
						...
						<ul class="list-unstyled text-white-50">
							...
							<li><strong>Estado actual:</strong> {{ ucfirst($order->status) }}</li>
							...
							<div class="input-group">
								<select name="status" class="form-select form-select-sm">
									...
									@foreach($states as $k => $label)
										<option value="{{ $k }}" {{ ($order->status === $k) || (empty($order->status) && $k === 'paid') ? 'selected' : '' }}>{{ $label }}</option>
									@endforeach
								</select>
					...
					<div class="col-12 col-md-6">
						...
						@php $payment = $order->payments()->orderBy('id','desc')->first(); $invoice = $order->invoice; @endphp
						@if($payment)
	...
	btn.addEventListener('click', function(e){
		...
		fetch("{{ url('/settings/orders') }}/"+orderId+"/invoice/ajax", {
			...
			credentials: 'same-origin'
				...
				else alert('No se pudo generar: ' + (data.message || 'Error'));
			}
		}).catch(err => {
			btn.disabled = false; btn.textContent = 'Generar y descargar XML';
			if (msg) { msg.style.display='block'; msg.textContent = 'Error de red al generar factura'; }
			else alert('Error de red al generar factura');
		});
	});
});
</script>
